...
# $CROSS_PREFIX-gcc -nostartfiles -T./link.lds -Wl,-Map=$SHELL_FOLDER/output/trusted_domain/trusted_domain_fw.map -Wl,--gc-sections $SHELL_FOLDER/output/trusted_domain/startup.o -o $SHELL_FOLDER/output/trusted_domain/trusted_domain_fw.elf
# # 使用gnu工具生成原始的程序bin文件
# $CROSS_PREFIX-objcopy -O binary -S $SHELL_FOLDER/output/trusted_domain/trusted_domain_fw.elf $SHELL_FOLDER/output/trusted_domain/trusted_domain_fw.bin
# # 使用gnu工具生成反汇编文件，方便调试分析（当然我们这个代码太简单，不是很需要）
# $CROSS_PREFIX-objdump --source --demangle --disassemble --reloc --wide $SHELL_FOLDER/output/trusted_domain/trusted_domain_fw.elf > $SHELL_FOLDER/output/trusted_domain/trusted_domain_fw.lst
...
# # 生成sbi.dtb
# cd $SHELL_FOLDER/dts
# dtc -I dts -O dtb -Wno-interrupts_extended_property -o $SHELL_FOLDER/output/opensbi/quard_star_sbi.dtb quard_star_sbi.dts
...
make
mv $SHELL_FOLDER/os/os.bin $SHELL_FOLDER/output/os/os.bin
make clean
...
dd of=fw.bin bs=1k conv=notrunc seek=4K if=$SHELL_FOLDER/output/trusted_domain/trusted_domain_fw.bin
